<?php /* Template_ 2.2.7 2015/11/23 00:40:49 /cloud9/workspace/cikorea/front_end/themes/pc/default/_template/contents/bbs/write.tpl 000009447 */ 
$TPL_category_1=empty($TPL_VAR["category"])||!is_array($TPL_VAR["category"])?0:count($TPL_VAR["category"]);
$TPL_tags_post_1=empty($TPL_VAR["tags_post"])||!is_array($TPL_VAR["tags_post"])?0:count($TPL_VAR["tags_post"]);
$TPL_urls_post_1=empty($TPL_VAR["urls_post"])||!is_array($TPL_VAR["urls_post"])?0:count($TPL_VAR["urls_post"]);?>
...
<form method = "post" name="write_form" id="write_form" action = "<?php echo $TPL_VAR["BASE_URL"]?>bbs/write/<?php echo $TPL_VAR["bbs_id"]?>" data-ajax="false" onsubmit = "return (form_null_check('write_form', '<?php echo $TPL_VAR["form_null_check"]?>') && form_minimum_check('write_form', '<?php echo $TPL_VAR["form_minimum_check"]?>'));">
...
<?php if($TPL_VAR["BBS_SETTING_bbs_tags_used"]== 1){?>
        <tr>
            ...
            <td>
<?php if($TPL_tags_post_1){foreach($TPL_VAR["tags_post"] as $TPL_V1){?>
...
<?php }}?>
                ...
                <input type="text" id="tags_pc" name="tags_pc" value="" data-role="tagsinput" placeholder="↙" /> (<?php echo $TPL_VAR["lang"]["max"]?> : <?php echo $TPL_VAR["BBS_SETTING_bbs_tags_limit_count"]?>)
                <script>
...
<?php if($TPL_VAR["BBS_SETTING_bbs_urls_used"]== 1){?>
        <tr>
            ...
            <td>
<?php if($TPL_urls_post_1){foreach($TPL_VAR["urls_post"] as $TPL_V1){?>
...
<?php }?>
    ...
    <div class="pull-right">
        <button type="button" class="btn btn-small btn-success" onclick = "confirm_really('write_form');"><?php echo $TPL_VAR["lang"]["write"]?></button>
        <a href="<?php echo $TPL_VAR["BASE_URL"]?>bbs/lists/<?php echo $TPL_VAR["bbs_id"]?>?view_category=<?php echo $TPL_VAR["view_category"]?>&amp;lists_style=<?php echo $TPL_VAR["lists_style"]?>" class="btn btn-small"><?php echo $TPL_VAR["lang"]["lists"]?></a>
    </div>
</form>

<form id="hiddenForm" name="hidden_form">
    <input type="hidden" id="upload_allowed_extension" name="upload_allowed_extension" value="<?php echo join('\',\'',$TPL_VAR["upload_allowed_extension"])?>">
</form>
...
<!-- script type="text/javascript" src="<?php echo $TPL_VAR["FRONTEND_THIRD_PARTY"]?>tinymce/jquery.tinymce.min.js"></script-->
